...
export function renderPluginTable(snap) {
  ...
  const rows = snap.plugins.map((p) => {
    ...
    return el('tr', null,
      ...
      el('td', { class: 'center num' }, (p.manifest.downloadCount ?? 0).toLocaleString('en-US')),
      el('td', { class: 'center num' }, releaseDownloadsCell(p)),
  ...
  const head = el('tr', null,
    ...
    el('th', { class: 'center' }, 'Downloads'),
    el('th', { class: 'center', title: 'Downloads of the current stable release (+testing) via GitHub release assets' }, 'This ver.'),
